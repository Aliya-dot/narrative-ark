...
export function nextAppTheme(current: AppThemeId): AppThemeId {
  const currentIndex = APP_THEME_IDS.indexOf(current);
  return APP_THEME_IDS[(currentIndex + 1) % APP_THEME_IDS.length];
}
